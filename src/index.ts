...
server.registerTool(
  ...
  async ({ mint, wallet }) => {
    const earnings = await fetchApi(
      `/api/tokens/${mint}/earnings?wallet=${wallet}`
    );
    return {
        ...
        content: [
          {
            ...
            text: JSON.stringify({
              error:
                "DUMBMONEY_API_KEY not set. Set it as an environment variable to create tokens.",
            }),
          },
        ],
        isError: true,
  ...
  "register_agent",
  {
    ...
    inputSchema: {
      ...
      fee_wallet: z
        ...
        .optional()
        .describe(
          "Optional Solana wallet address to receive creator fees from tokens you create"
        ),
    },
  },
  async (params) => {
...
server.registerTool(
  ...
  async () => {
        ...
        content: [
          {
            ...
            text: JSON.stringify({
              error:
                "DUMBMONEY_API_KEY not set. Register first with register_agent, then set the returned API key.",
            }),
          },
        ],
        isError: true,
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Failed to get agent info";
      return {
        content: [{ type: "text", text: JSON.stringify({ error: message }) }],
        isError: true,
      };
